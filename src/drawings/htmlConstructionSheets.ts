...
export function buildBeamGroupLabels(beamDesigns: BeamDesignData[], bentUpResults?: any[]): Map<string, string> {
...
export function buildColGroupLabels(colDesigns: ColDesignData[]): Map<string, string> {
  const keyToLabel = new Map<string, string>();
  const result = new Map<string, string>();
  let counter = 1;
  for (const c of colDesigns) {
    if (!c) continue;
    const b = c.b ?? (c as any).b_width ?? 250;
    const h = c.h ?? (c as any).h_height ?? 600;
    const bars = c.design?.bars ?? (c as any).barCount ?? (c as any).design?.bars ?? 8;
    const dia = c.design?.dia ?? (c as any).barDiameter ?? (c as any).design?.dia ?? 16;
    const stirrups = c.design?.stirrups ?? (c as any).tiesText ?? (c as any).design?.stirrups ?? 'Ø10@150';
    const id = c.id ?? (c as any).columnId ?? '';
    const key = `${b}x${h}|${bars}φ${dia}|${stirrups}`;
    if (!keyToLabel.has(key)) {
      keyToLabel.set(key, `ع-${counter++}`);
    }
    result.set(id, keyToLabel.get(key)!);
  }
  return result;
}

/** Build a map of slabId → Arabic group label (ب-1, ب-2, …) by identical reinforcement pattern */
export function buildSlabGroupLabels(slabDesigns: SlabDesignData[]): Map<string, string> {
  const keyToLabel = new Map<string, string>();
  const result = new Map<string, string>();
  let counter = 1;
  for (const s of slabDesigns) {
    if (!s) continue;
    const hUsed = s.design?.hUsed ?? (s as any).thickness ?? (s.design as any)?.thick ?? 150;
    const sBars = s.design?.shortDir?.bars ?? 5;
    const sDia = s.design?.shortDir?.dia ?? 12;
    const sSpacing = s.design?.shortDir?.spacing ?? 150;
    const lBars = s.design?.longDir?.bars ?? 5;
    const lDia = s.design?.longDir?.dia ?? 12;
    const lSpacing = s.design?.longDir?.spacing ?? 150;
    const key = `h${hUsed}|${sBars}φ${sDia}@${sSpacing}|${lBars}φ${lDia}@${lSpacing}`;
    if (!keyToLabel.has(key)) {
...
function svgGridSystem(
  ...
  // 1. Dashed Grid Ax lines and outer bubbles for X positions
  for (let i = 0; i < gridX.length; i++) {
    const x = tx(gridX[i]);
    const yTop = ty(maxY + 0.22);
    const yBot = ty(minY - 0.22);
    
    // Chain-dashed grid line
    svg += `<line x1="${x}" y1="${yBot}" x2="${x}" y2="${yTop}" stroke="rgba(100, 116, 139, 0.45)" stroke-width="0.32" stroke-dasharray="4,2.5" />`;
    
    // Top Bubble
    const byTop = ty(maxY + 0.32);
    svg += `<circle cx="${x}" cy="${byTop}" r="11" fill="white" stroke="#475569" stroke-width="0.8" />`;
    svg += `<text x="${x}" y="${byTop + 3.2}" text-anchor="middle" font-size="8.5" font-weight="bold" font-family="Arial" fill="#0f172a">${xLabels[i]}</text>`;
    
    // Bottom Bubble
    const byBot = ty(minY - 0.32);
    svg += `<circle cx="${x}" cy="${byBot}" r="11" fill="white" stroke="#475569" stroke-width="0.8" />`;
    svg += `<text x="${x}" y="${byBot + 3.2}" text-anchor="middle" font-size="8.5" font-weight="bold" font-family="Arial" fill="#0f172a">${xLabels[i]}</text>`;
  }
  
  // 2. Dashed Grid Ax lines and outer bubbles for Y positions
  for (let i = 0; i < gridY.length; i++) {
    const y = ty(gridY[i]);
    const xLeft = tx(minX - 0.22);
    const xRight = tx(maxX + 0.22);
    
    // Chain-dashed grid line
    svg += `<line x1="${xLeft}" y1="${y}" x2="${xRight}" y2="${y}" stroke="rgba(100, 116, 139, 0.45)" stroke-width="0.32" stroke-dasharray="4,2.5" />`;
    
    // Left Bubble
    const bxLeft = tx(minX - 0.32);
    svg += `<circle cx="${bxLeft}" cy="${y}" r="11" fill="white" stroke="#475569" stroke-width="0.8" />`;
    svg += `<text x="${bxLeft}" y="${y + 3.2}" text-anchor="middle" font-size="8.5" font-weight="bold" font-family="Arial" fill="#0f172a">${yLabels[i]}</text>`;
    
    // Right Bubble
    const bxRight = tx(maxX + 0.32);
    svg += `<circle cx="${bxRight}" cy="${y}" r="11" fill="white" stroke="#475569" stroke-width="0.8" />`;
    svg += `<text x="${bxRight}" y="${y + 3.2}" text-anchor="middle" font-size="8.5" font-weight="bold" font-family="Arial" fill="#0f172a">${yLabels[i]}</text>`;
  }

  // 3. Horizontal nested span dimension lines between adjacent X-Grids
  for (let i = 0; i < gridX.length - 1; i++) {
    const xStart = tx(gridX[i]);
    const xEnd = tx(gridX[i + 1]);
    const dist = gridX[i + 1] - gridX[i];
    const yLine = ty(maxY + 0.13);
    
    svg += `<line x1="${xStart}" y1="${yLine}" x2="${xEnd}" y2="${yLine}" stroke="#ec4899" stroke-width="0.4" />`;
    svg += `<line x1="${xStart}" y1="${yLine - 2}" x2="${xStart}" y2="${yLine + 2}" stroke="#ec4899" stroke-width="0.4" />`;
    svg += `<line x1="${xEnd}" y1="${yLine - 2}" x2="${xEnd}" y2="${yLine + 2}" stroke="#ec4899" stroke-width="0.4" />`;
    svg += `<line x1="${xStart - 1.5}" y1="${yLine - 1.5}" x2="${xStart + 1.5}" y2="${yLine + 1.5}" stroke="#be185d" stroke-width="0.75" />`;
    svg += `<line x1="${xEnd - 1.5}" y1="${yLine - 1.5}" x2="${xEnd + 1.5}" y2="${yLine + 1.5}" stroke="#be185d" stroke-width="0.75" />`;
    svg += `<text x="${(xStart + xEnd) / 2}" y="${yLine - 3}" text-anchor="middle" font-size="7" font-weight="bold" font-family="monospace" fill="#db2777">${dist.toFixed(2)} m</text>`;
  }

  // 4. Vertical nested span dimension lines between adjacent Y-Grids
  for (let i = 0; i < gridY.length - 1; i++) {
    const yStart = ty(gridY[i]);
    const yEnd = ty(gridY[i + 1]);
    const dist = gridY[i + 1] - gridY[i];
    const xLine = tx(minX - 0.13);
    
    svg += `<line x1="${xLine}" y1="${yStart}" x2="${xLine}" y2="${yEnd}" stroke="#ec4899" stroke-width="0.4" />`;
    svg += `<line x1="${xLine - 2}" y1="${yStart}" x2="${xLine + 2}" y2="${yStart}" stroke="#ec4899" stroke-width="0.4" />`;
    svg += `<line x1="${xLine - 2}" y1="${yEnd}" x2="${xLine + 2}" y2="${yEnd}" stroke="#ec4899" stroke-width="0.4" />`;
    svg += `<line x1="${xLine - 1.5}" y1="${yStart + 1.5}" x2="${xLine + 1.5}" y2="${yStart - 1.5}" stroke="#be185d" stroke-width="0.75" />`;
    svg += `<line x1="${xLine - 1.5}" y1="${yEnd + 1.5}" x2="${xLine + 1.5}" y2="${yEnd - 1.5}" stroke="#be185d" stroke-width="0.75" />`;
    svg += `<text x="${xLine - 4}" y="${(yStart + yEnd) / 2 + 2.5}" text-anchor="end" font-size="7" font-weight="bold" font-family="monospace" fill="#db2777">${dist.toFixed(2)} m</text>`;
  }
...
export function htmlTitleBlock(config: Partial<TitleBlockConfig>): string {
  return `
...
export function htmlSheetBorder(): string {
  return `
...
export function htmlBeamScheduleTable(beams: Beam[], beamDesigns: BeamDesignData[], bentUpResults?: any[]): string {
...
export function htmlColumnScheduleTable(colDesigns: ColDesignData[]): string {
...
export function htmlSlabScheduleTable(slabDesigns: SlabDesignData[], slabs: Slab[]): string {
...
export function generateHTMLConstructionSheets(
  ...
  const mmPerM = Math.min((planZoneW - 110) / modelW, (svgH - 110) / modelH);
  const planOffsetX = 55 + ((planZoneW - 110) - modelW * mmPerM) / 2;
  const planOffsetY = 55 + ((svgH - 110) - modelH * mmPerM) / 2;
  const tx = (x: number) => (x - minX) * mmPerM + planOffsetX;
  const ty = (y: number) => (maxY - y) * mmPerM + planOffsetY;

  const activeColsForGrids = columns.filter(c => !c.isRemoved);
  const gridX = activeColsForGrids.length > 0 
    ? Array.from(new Set(activeColsForGrids.map(c => Number(c.x.toFixed(2))))).sort((a, b) => a - b)
    : Array.from(new Set(allX)).sort((a, b) => a - b);
  const gridY = activeColsForGrids.length > 0 
    ? Array.from(new Set(activeColsForGrids.map(c => Number(c.y.toFixed(2))))).sort((a, b) => a - b)
    : Array.from(new Set(allY)).sort((a, b) => a - b);
  ...
  const minZ = fndCols.length > 0 ? Math.min(...fndCols.map((c: any) => c.zBottom ?? 0)) : 0;
  const groundCols = fndCols.filter((col: any) => Math.abs((col.zBottom ?? 0) - minZ) < 100);
  const validCols = groundCols.length > 0 ? groundCols : fndCols;

  if (hasAnyFnd && validCols.length > 0) {
    if (true) {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Super compact tables to prevent overflow */
    table td, table th { 
    ...
    /* Adjust text labels on the right side schedules */
    div[style*="font-size:11px"] {
      font-size: 9px !important;
      margin-bottom: 2px !important;
    }
    div[style*="font-size:7.5px"], div[style*="font-size:7px"] {
