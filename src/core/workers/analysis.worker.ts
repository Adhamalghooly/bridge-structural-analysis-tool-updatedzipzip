...
async function runAnalysis(input: AnalysisInput): Promise<void> {
      ...
      if (detectedConnections.length > 0) {
        ...
        if (hasBobFrame) {
          ...
          const results3D = getFrameResults3D(
            ...
            slabs, slabProps, payload.useFEMLoadDistribution,
            beamStiffnessFactor, colStiffnessFactor,
  ...
  const supportReactions = (frameResults as any).supportReactions || [];

  send({ type: 'FINAL_RESULT', frameResults, bobConnections, diagnostics, supportReactions });
}
