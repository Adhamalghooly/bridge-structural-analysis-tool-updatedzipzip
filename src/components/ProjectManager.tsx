...
import { dbStore } from '@/lib/indexedDbStore';
...
export default function ProjectManager({
  ...
  const saveProjects = (updated: SavedProject[]) => {
    dbStore.setItem(STORAGE_KEY, updated);
    setProjects(updated);
  };

  const setActiveProjectId = (id: string | null) => {
    if (id) {
      dbStore.setItem(ACTIVE_KEY, id);
    } else {
      dbStore.removeItem(ACTIVE_KEY);
    }
    setActiveId(id);
  };
  ...
  useEffect(() => {
    const loadFromIndexedDB = async () => {
      let saved = await dbStore.getItem<SavedProject[]>(STORAGE_KEY);
      if (!saved) {
        try {
          const raw = localStorage.getItem(STORAGE_KEY);
          if (raw) {
            saved = JSON.parse(raw);
            await dbStore.setItem(STORAGE_KEY, saved);
          }
        } catch (_) {}
      }
      setProjects(saved || []);

      let activeId = await dbStore.getItem<string>(ACTIVE_KEY);
      if (!activeId) {
        try {
          activeId = localStorage.getItem(ACTIVE_KEY);
          if (activeId) {
            await dbStore.setItem(ACTIVE_KEY, activeId);
          }
        } catch (_) {}
      }
  ...
  const getSerializableState = useCallback(() => {
    const { undoStack, modalOpen, selectedElement, elemPropsOpen, diagramOpen, diagramData, savedMessage, elemPropsFrameId, elemPropsAreaId, ...rest } = currentState;
    return {
      ...rest,
      _exportVersion: 2,
