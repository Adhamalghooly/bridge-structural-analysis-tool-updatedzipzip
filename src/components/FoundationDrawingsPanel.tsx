...
export default function FoundationDrawingsPanel({
  ...
  const qualityControlWarnings = useMemo(() => {
    ...
    // Real ACI 318 USD engineering failures per column instance (bearing/one-way shear/
    // punching shear/development length) — previously never surfaced anywhere in the UI.
    projectData.locations.forEach(loc => {
      if (!loc.adequate) {
        const failed: string[] = [];
        if (!loc.bearingOk) failed.push('تجاوز إجهاد التربة المسموح');
        if (!loc.wideShearOk) failed.push('قص عرضي (One-way shear)');
        if (!loc.punchingOk) failed.push('قص ثاقب (Punching shear)');
        if (!loc.developmentOk) failed.push('طول التطوير (Development length)');
        warnings.push(`⚠ فشل تصميم: العمود ${loc.colId} (${loc.typeMark}) — ${failed.join('، ') || 'راجع نتائج التصميم التفصيلية'}.`);
      }
    });
  ...
  const handleDXFExport = () => {
    const dxfInputs: FoundationDXFInput[] = projectData.types.flatMap((type) => {
      ...
      return locsOfType.map(loc => ({
        ...
        spacing_x: type.rebarX.spacing,
        bars_y: type.rebarY.quantity,
        dia_y: type.rebarY.diameter,
        spacing_y: type.rebarY.spacing,
        // Real per-instance ACI 318 USD status from generateProjectDetailing (analysis+design),
        // not a hardcoded placeholder — loads differ per column even for identical geometry.
        bearing_ok: loc.bearingOk,
        wide_shear_ok: loc.wideShearOk,
        punch_shear_ok: loc.punchingOk,
        adequate: loc.adequate
      }));
    });
