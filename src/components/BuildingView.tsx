...
interface BuildingViewProps {
  ...
  onSelectElement?: (type: 'beam' | 'column' | 'slab' | 'node', id: string) => void;
...
export default function BuildingView({
  slabs, beams, columns, nodes, analyzed, frameResults, beamDesigns, onSelectElement, onLongPressSlab,
  ...
  const activeNodes = React.useMemo(() => {
    ...
    return nodes.filter(n => {
      const matchesBeam = beams.some(b => 
        (Math.abs(b.x1 - n.x) < EPS && Math.abs(b.y1 - n.y) < EPS) ||
        (Math.abs(b.x2 - n.x) < EPS && Math.abs(b.y2 - n.y) < EPS)
      );
      const matchesCol = columns.some(c =>
        Math.abs(c.x - n.x) < EPS && Math.abs(c.y - n.y) < EPS
      );
      return matchesBeam || matchesCol;
    });
  }, [nodes, beams, columns]);
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto max-h-[60vh] md:max-h-[70vh]" style={{ background: 'hsl(var(--background))' }}>
      ...
      {nodes && activeNodes.map(n => {
        ...
        const hasRestraint = n.restraints && (
          n.restraints.ux || n.restraints.uy || n.restraints.uz ||
          n.restraints.rx || n.restraints.ry || n.restraints.rz
        );

        let nodeColor = "hsl(var(--muted-foreground) / 0.5)";
        let strokeColor = "hsl(var(--foreground) / 0.5)";
        let radius = 3.5;
        let isSupport = false;
        ...
        if (hasRestraint) {
          ...
          const isFixed = r.ux && r.uy && r.uz && r.rx && r.ry && r.rz;
          const isPinned = r.ux && r.uy && r.uz && !r.rx && !r.ry && !r.rz;
          const isRoller = !r.ux && !r.uy && r.uz;
          
          if (isFixed) {
            nodeColor = "rgb(239, 68, 68)";
            strokeColor = "rgb(153, 27, 27)";
            radius = 5.5;
          } else if (isPinned) {
            nodeColor = "rgb(249, 115, 22)";
            strokeColor = "rgb(194, 65, 12)";
            radius = 5;
          } else if (isRoller) {
        ...
        return (
          <g key={`view-node-${n.id}`} className="cursor-pointer select-none group">
            {/* Larger click area for touch devices */}
            <circle
              cx={cx}
              cy={cy}
              r="14"
              fill="transparent"
              onClick={() => onSelectElement?.('node', String(n.id))}
            />
            ...
            <title>{`العقدة N${n.id}${hasRestraint ? ' (ركيزة)' : ''}`}</title>
          </g>
        );
      })}
