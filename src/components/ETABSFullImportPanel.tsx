    ...
    try {
      ...
      if (type === 'nodes') {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const sec = row.length > 3 ? String(row[3] ?? '').trim() : undefined;
          if (!id || !ni || !nj) continue;
          imported.push({ id, story: '', nodeI: ni, nodeJ: nj, section: sec });
        }
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (row.length > 5) {
             const t = parseFloat(String(row[5]));
             if (!isNaN(t)) thickness = t;
          }
          if (slabNodes.length < 3) continue;
          imported.push({ id, story: '', nodes: slabNodes, thickness });
        }
        setSlabs(imported);
