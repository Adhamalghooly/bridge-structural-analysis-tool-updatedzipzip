...
export default function StripFootingAnalysisPanel({
  ...
  // --- MULTI-STRIP FOOTING MANAGEMENT STACK ---
  const [localStripFootings, setLocalStripFootings] = useState<any[]>(() => [
    {
      id: 'sf-1',
      name: 'أساس مستمر SF1',
      L: 8000,
  ...
  const activeFootingsList = stripFootings || localStripFootings;
  const updateFootingsList = onStripFootingsChange || setLocalStripFootings;

  const [activeStripId, setActiveStripId] = useState<string>(activeFootingsList[0]?.id || 'sf-1');
  const lastLoadedStripIdRef = React.useRef<string | null>(null);

  // --- GROUND COLUMNS RESOLVER FOR STRIP MODELING ---
  const groundCols = useMemo(() => {
    if (supportDb && supportDb.assignments && supportDb.assignments.length > 0) {
      const assignedNodeIds = new Set(supportDb.assignments.map((a: any) => a.nodeId));
  ...
  // --- AUTOMATIC ALIGNMENT PROPOSALS ENGINE ---
  const [selectedProposalIds, setSelectedProposalIds] = useState<string[]>([]);
  const [proposalColumnSelections, setProposalColumnSelections] = useState<Record<string, string[]>>({});

  // 1. Identify alignment proposals
  const alignmentProposals = useMemo(() => {
    if (!groundCols || groundCols.length < 2) return [];

    const isMMS = Math.max(...groundCols.map(c => Math.abs(c.x))) > 100;
    const tol = isMMS ? 350 : 0.35;
    const factor = isMMS ? 1000 : 1;
    ...
    // Group Horizontally: columns sharing same Y coordinate (within tol)
    const horizontalGroups: Column[][] = [];
    const processedY = new Set<number>();

    groundCols.forEach(col => {
      const isProcessed = [...processedY].some(y => Math.abs(col.y - y) <= tol);
      if (isProcessed) return;

      const group = groundCols.filter(c => Math.abs(c.y - col.y) <= tol);
      if (group.length >= 2) {
        group.sort((a, b) => a.x - b.x);
        horizontalGroups.push(group);
        processedY.add(col.y);
      }
    });

    // Group Vertically: columns sharing same X coordinate (within tol)
    const verticalGroups: Column[][] = [];
    const processedX = new Set<number>();

    groundCols.forEach(col => {
      const isProcessed = [...processedX].some(x => Math.abs(col.x - x) <= tol);
      if (isProcessed) return;

      const group = groundCols.filter(c => Math.abs(c.x - col.x) <= tol);
      if (group.length >= 2) {
        group.sort((a, b) => a.y - b.y);
        verticalGroups.push(group);
        processedX.add(col.x);
      }
    });

    let proposalIndex = 1;

    const buildProposals = (groups: Column[][], type: 'horizontal' | 'vertical') => {
      groups.forEach(group => {
        const uniqueCols = group.filter((v, i, a) => a.findIndex(t => t.id === v.id) === i);
        if (uniqueCols.length < 2) return;

        const columnIdsString = uniqueCols.map(c => c.id).sort().join(',');
        
        const exists = proposals.some(p => p.columns.map(c => c.id).sort().join(',') === columnIdsString);
        if (exists) return;

        const avgCoord = uniqueCols.reduce((sum, c) => sum + (type === 'horizontal' ? c.y : c.x), 0) / uniqueCols.length;
        const coordInMeters = avgCoord / factor;

        const name = type === 'horizontal'
          ? `قاعدة مستمرة أفقية (أفقي) SF-H${proposalIndex++}`
          : `قاعدة مستمرة رأسية (رأسي) SF-V${proposalIndex++}`;

        proposals.push({
          id: `prop-${type}-${proposalIndex}`,
          type,
          coordinateValue: coordInMeters,
          columns: uniqueCols,
          name
        });
      });
    ...
    buildProposals(horizontalGroups, 'horizontal');
    buildProposals(verticalGroups, 'vertical');

    return proposals;
  }, [groundCols]);

  // Sync default column selections inside each proposal
  useEffect(() => {
    setProposalColumnSelections(prev => {
      const updated = { ...prev };
      let changed = false;
      alignmentProposals.forEach(prop => {
        if (!updated[prop.id]) {
          updated[prop.id] = prop.columns.map(c => c.id);
          changed = true;
        }
      });
  ...
  // Helper to generate a single new footing configuration object
  const handleApplyColumnsAsNewFooting = useCallback((columnIds: string[], footingName: string) => {
    if (!columns || columns.length === 0 || columnIds.length === 0) return null;

    const selectedCols = columns.filter(col => columnIds.includes(col.id));
    ...
    const mappedLoads: StripFootingLoad[] = sortedCols.map((col) => {
      ...
      const key = `${col.x.toFixed(2)}_${col.y.toFixed(2)}_${col.zBottom ?? 0}`;
      if (supportDb?.reactions) {
        const matchingReactions = supportDb.reactions.filter((r: any) => r.nodeId === key);
        if (matchingReactions.length > 0) {
          const chosen = matchingReactions.find((r: any) => r.loadCase === '1.0D+1.0L') || matchingReactions[0];
          fz = Math.abs(chosen.fz);
          mx = chosen.mx;
          my = chosen.my;
        }
      }
      ...
      const PDead = parseFloat((fz * 0.6).toFixed(1));
      const PLive = parseFloat((fz * 0.4).toFixed(1));

      const MDead = mx ? parseFloat((mx * 0.6).toFixed(1)) : 0;
      const MLive = mx ? parseFloat((mx * 0.4).toFixed(1)) : 0;

      return {
        id: `auto-${col.id}-${Date.now()}-${Math.floor(Math.random() * 100)}`,
        type: 'column',
        label: col.id,
    ...
    const newId = `sf-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    return {
      id: newId,
      name: footingName,
      L: Math.max(1000, Math.round(calculatedLengthM * 1000)),
      B: 1600,
      H: 650,
      Ks: 25000,
      loads: mappedLoads,
  ...
  }, [columns, supportDb, colLoads3D]);

  // Query active footing record
  const activeStrip = useMemo(() => {
    return activeFootingsList.find(f => f.id === activeStripId) || activeFootingsList[0];
  }, [activeFootingsList, activeStripId]);
  ...
  const updateActiveStripProperty = useCallback((updates: { L?: number; B?: number; H?: number; Ks?: number; loads?: StripFootingLoad[] }) => {
    if (!activeStrip) return;
    if (updates.L !== undefined) setL(updates.L);
    if (updates.B !== undefined) setB(updates.B);
    if (updates.H !== undefined) setH(updates.H);
    if (updates.Ks !== undefined) setKs(updates.Ks);
    if (updates.loads !== undefined) setLoads(updates.loads);

    const updatedList = activeFootingsList.map(f => {
      if (f.id === activeStripId) {
        return {
          ...f,
          L: updates.L !== undefined ? updates.L : f.L,
          B: updates.B !== undefined ? updates.B : f.B,
          H: updates.H !== undefined ? updates.H : f.H,
          Ks: updates.Ks !== undefined ? updates.Ks : f.Ks,
          loads: updates.loads !== undefined ? updates.loads : f.loads,
        };
      }
      return f;
    });
    updateFootingsList(updatedList);
  }, [activeStrip, activeStripId, activeFootingsList, updateFootingsList]);

  // Sync internal hooks whenever active footing changes
  useEffect(() => {
    if (activeStrip && activeStrip.id !== lastLoadedStripIdRef.current) {
      setL(activeStrip.L);
      setB(activeStrip.B);
      setH(activeStrip.H);
      setKs(activeStrip.Ks ?? 25000);
      setLoads(activeStrip.loads);
      lastLoadedStripIdRef.current = activeStrip.id;
    }
  }, [activeStrip, activeStripId]);
  ...
  // --- REAL-TIME GEOTECHNICAL SOIL SETTLEMENTS ---
  const stripSettlementResult = useMemo(() => {
    try {
      const pts = loads.map(l => ({
        x: l.x,
        P: (l.PDead ?? 0) + (l.PLive ?? 0)
      }));
      
      const geoParams: GeotechnicalParameters = {
        qall,
        Ks,
        Es: 25, // default standard soil modulus (MPa)
        poisson: 0.3,
        embedmentDepth: 1.5,
      ...
      console.error("Geotechnical continuous settlement evaluation failed:", err);
      return null;
    }
  }, [L, B, H, qall, Ks, loads, activeStrip?.name]);
  ...
  // --- COMPUTE SUPPORT NODES AND ALIGN WITH SUPPORT DATABASE ---
  const computedNodesPool = useMemo(() => {
    ...
    columns.filter(cc => !cc.isRemoved).forEach(c => {
      ...
      if (!nodeMap.has(keyTop)) {
        nodeMap.set(keyTop, { id: '', x: c.x, y: c.y, z: zTop });
      }
      if (!nodeMap.has(keyBot)) {
        nodeMap.set(keyBot, { id: '', x: c.x, y: c.y, z: zBot });
      }
    });
    ...
    // Sort to match SupportManagerPanel's sorting
  ...
  const getColNodeLabel = (col: Column) => {
    const matched = computedNodesPool.find(n => 
      Math.abs(n.x - col.x) < 0.05 && 
      Math.abs(n.y - col.y) < 0.05 && 
      Math.abs(n.z - (col.zBottom ?? 0)/1000) < 0.05
    );
  ...
  const handleImportSelectedColumns = (idsToImport: string[]) => {
    ...
    const mappedLoads: StripFootingLoad[] = sortedCols.map((col) => {
      ...
      // 1. Try to fetch from support reactions database using coordinate key
      const key = `${col.x.toFixed(2)}_${col.y.toFixed(2)}_${col.zBottom ?? 0}`;
      if (supportDb?.reactions) {
        const matchingReactions = supportDb.reactions.filter((r: any) => r.nodeId === key);
        if (matchingReactions.length > 0) {
          // Prefer active/standard case (like 1.0D+1.0L) or fallback to first case
          const chosen = matchingReactions.find((r: any) => r.loadCase === '1.0D+1.0L') || matchingReactions[0];
          fz = Math.abs(chosen.fz);
          // In strip footing coordinate space, moment is mapped from Mx/My
          mx = chosen.mx;
          my = chosen.my;
        }
      }

      // 2. Fallback to colLoads3D if not found in support reactions
      if (fz === 0) {
        const loads3D = colLoads3D?.get(col.id);
        const P_service = loads3D?.P_service
          ? parseFloat(loads3D.P_service.toFixed(1))
  ...
  const handleAutoImportFromModel = () => {
    if (!groundCols || groundCols.length === 0) return;
  ...
  const handleLoadBenchmark = (bench: typeof benchmarksList[0]) => {
    ...
    updateActiveStripProperty({
      ...
      H: bench.input.H,
      Ks: bench.input.Ks,
      loads: bench.input.loads as any
    });
  };
  ...
  return (
    <div className="space-y-6 font-sans text-right" id="stripFootingAnalysisSection" style={{ direction: 'rtl' }}>
      ...
      {/* MULTI-STRIP MODEL MANAGER BAR */}
      <Card className="p-3.5 bg-muted/20 border-border">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4" dir="rtl">
          <div className="flex items-center gap-2.5">
            <span className="text-xs font-bold text-slate-700 dark:text-slate-200 shrink-0">الموديل النشط للقاعدة الشريطية:</span>
            <select
              value={activeStripId}
              onChange={(e) => setActiveStripId(e.target.value)}
              className="h-9 px-3 text-xs bg-background border rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 font-bold text-blue-700 dark:text-blue-400 font-mono"
            >
              {activeFootingsList.map(sf => (
                <option key={sf.id} value={sf.id}>
                  {sf.name} ({sf.L}×{sf.B} مم)
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Button
              ...
              onClick={() => {
                ...
                const newName = `أساس مستمر SF-${activeFootingsList.length + 1}`;
                const added = {
                  id: newId,
                  name: newName,
                  L: 9000,
                  ...
                  Ks: 25000,
                  loads: [
                    { id: 'col-1', type: 'column', label: 'C1', x: 1.5, PDead: 350, PLive: 150, MDead: 10, MLive: 5, columnCx: 400, columnCy: 400 },
                    { id: 'col-2', type: 'column', label: 'C2', x: 4.5, PDead: 420, PLive: 180, MDead: 0, MLive: 0, columnCx: 400, columnCy: 400 },
                    { id: 'col-3', type: 'column', label: 'C3', x: 7.5, PDead: 350, PLive: 150, MDead: -10, MLive: -5, columnCx: 400, columnCy: 400 },
                  ],
                  selectedColumnIds: []
                };
                const updatedList = [...activeFootingsList, added];
                updateFootingsList(updatedList);
            ...
            <Button
              ...
              className="h-8 text-[11px] font-bold gap-1 cursor-pointer"
              onClick={() => {
                const curName = activeStrip?.name || '';
                const newName = prompt('أدخل الاسم الجديد للقاعدة الشريطية:', curName);
                if (newName && newName.trim()) {
                  const updatedList = activeFootingsList.map(f => {
                    if (f.id === activeStripId) return { ...f, name: newName.trim() };
                    return f;
                  });
                  updateFootingsList(updatedList);
                }
            ...
            >
              ...
              تعديل مسمّى خط التأسيس
            </Button>

            {activeFootingsList.length > 1 && (
              <Button
                size="sm"
                variant="destructive"
                type="button"
                className="h-8 text-[11px] font-bold gap-1 cursor-pointer"
                onClick={() => {
                  if (confirm('هل أنت متأكد من رغبتك في حذف هذا الموديل بالكامل؟')) {
                    const idx = activeFootingsList.findIndex(f => f.id === activeStripId);
                    const updatedList = activeFootingsList.filter(f => f.id !== activeStripId);
                    const nextActive = updatedList[Math.max(0, idx - 1)]?.id || updatedList[0]?.id;
                    if (nextActive) setActiveStripId(nextActive);
                    updateFootingsList(updatedList);
                  }
                }}
              >
      ...
      {activeMainTab === 'analysis' && (
        ...
        <div className="xl:col-span-5 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input 
                    ...
                    step="100" 
                    value={L} 
                    onChange={e => updateActiveStripProperty({ L: Math.max(500, parseInt(e.target.value) || 0) })} 
                ...
                <div className="space-y-1.5">
                  ...
                  <Input 
                    ...
                    step="50" 
                    value={B} 
                    onChange={e => updateActiveStripProperty({ B: Math.max(300, parseInt(e.target.value) || 0) })} 
                    className="h-8 text-xs font-mono"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input 
                    ...
                    step="50" 
                    value={H} 
                    onChange={e => updateActiveStripProperty({ H: Math.max(150, parseInt(e.target.value) || 0) })} 
                    className="h-8 text-xs font-mono"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input 
                    ...
                    step="1000"
                    value={Ks} 
                    onChange={e => updateActiveStripProperty({ Ks: Math.max(500, parseInt(e.target.value) || 20000) })} 
                    className="h-8 text-xs font-mono"
                  />
                </div>
              </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4 text-slate-800 dark:text-slate-200 font-mono text-[10.5px]">
                            ...
                            <span className="font-semibold text-teal-700 dark:text-teal-450">Z: {((col.zBottom ?? 0) / 1000).toFixed(2)}م</span>
          ...
          {/* D. AUTOMATIC STRIP FOOTING ALIGNMENT PROPOSALS */}
          {columns && columns.length > 0 && alignmentProposals.length > 0 && (
            <Card className="border border-emerald-200 dark:border-emerald-900 bg-emerald-50/5">
              <CardHeader className="py-3 bg-emerald-500/10 border-b border-emerald-200 dark:border-emerald-900">
                <CardTitle className="text-xs font-bold flex items-center gap-1.5 justify-start text-emerald-800 dark:text-emerald-400">
                  <Calculator className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                  مستشار اقتراحات توليد القواعد الشريطية التلقائي (Strip Footing Suggestion Adviser)
                </CardTitle>
                <CardDescription className="text-[11px] text-muted-foreground mr-1 text-right">
                  قائمة بالقواعد الشريطية المستمرة المكتشفة تلقائياً بناءً على استقامة الأعمدة على محور واحد أفقياً أو رأسياً.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-4 space-y-3 text-xs">
                <p className="text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed text-justify bg-emerald-500/5 p-2.5 rounded border border-emerald-500/20">
                  💡 <strong>التوليد التلقائي الذكي:</strong> تم الكشف التلقائي عن خطوط الأعمدة المستقيمة. يمكنك استبعاد بعض الأعمدة من القاعدة، أو دمج القواعد المحددة وتوليدها بضغطة زر واحدة.
                </p>

                <div className="overflow-x-auto border rounded-lg bg-background">
                  <table className="w-full text-right border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-900 border-b border-border text-muted-foreground font-bold text-[11px]">
                        <th className="p-2 w-10 text-center">
                          <input 
                            type="checkbox"
                            checked={selectedProposalIds.length === alignmentProposals.length && alignmentProposals.length > 0}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedProposalIds(alignmentProposals.map(p => p.id));
                              } else {
                        ...
                        <th className="p-2">تسمية الموديل المقترح</th>
                        <th className="p-2">الأعمدة المشمولة (انقر للاستبعاد/التضمين)</th>
                        <th className="p-2 text-center">أبعاد تقديرية</th>
                        <th className="p-2 text-center">إجمالي الأحمال</th>
                        <th className="p-2 text-center">الإجراء</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {alignmentProposals.map(prop => {
                        const isPropChecked = selectedProposalIds.includes(prop.id);
                        const activeColsInProp = proposalColumnSelections[prop.id] || prop.columns.map(c => c.id);
                        
                        // Calculate metrics for current selection
                        const selectedColumnsCount = activeColsInProp.length;
                        
                        // Estimate dimensions
                        let estimatedL = 0;
                        let totalP = 0;
                        if (selectedColumnsCount > 0) {
                          const footingObj = handleApplyColumnsAsNewFooting(activeColsInProp, prop.name);
                          if (footingObj) {
                            estimatedL = footingObj.L;
                            totalP = footingObj.loads.reduce((sum, l) => sum + (l.PDead || 0) + (l.PLive || 0), 0);
                          }
                        }
                        ...
                        return (
                          <tr key={prop.id} className="hover:bg-muted/30 transition-colors">
                            <td className="p-2 text-center align-middle">
                              <input 
                                type="checkbox"
                                checked={isPropChecked}
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setSelectedProposalIds([...selectedProposalIds, prop.id]);
                                  } else {
                                    setSelectedProposalIds(selectedProposalIds.filter(id => id !== prop.id));
                                  }
                                }}
                                className="rounded border-slate-300 text-emerald-600 h-3.5 w-3.5 cursor-pointer"
                              />
                            </td>
                            <td className="p-2 align-middle font-semibold text-foreground">
                              <div className="flex flex-col gap-0.5">
                                <span className="font-bold">{prop.name}</span>
                                <span className={`text-[9.5px] px-1.5 py-0.5 rounded w-max font-medium ${
                                  prop.type === 'horizontal' 
                                    ? 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300' 
                                    : 'bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300'
                                }`}>
                                  {prop.type === 'horizontal' ? 'أفقي ↔' : 'رأسي ↕'} (محور: {prop.coordinateValue.toFixed(2)}م)
                                </span>
                              </div>
                            </td>
                            <td className="p-2 align-middle">
                              <div className="flex flex-wrap gap-1.5">
                                {prop.columns.map(col => {
                                  const isColChecked = activeColsInProp.includes(col.id);
                                  return (
                                    <button
                                      key={col.id}
                                      type="button"
                                      onClick={() => {
                                        const currentSel = proposalColumnSelections[prop.id] || prop.columns.map(c => c.id);
                                        const newSel = currentSel.includes(col.id)
                                          ? currentSel.filter(id => id !== col.id)
                                          : [...currentSel, col.id];
                                        setProposalColumnSelections({
                                          ...proposalColumnSelections,
                                          [prop.id]: newSel
                                        });
                                      }}
                                      className={`px-2 py-0.5 rounded border text-[10px] font-bold flex items-center gap-1.5 transition-all ${
                                        isColChecked
                                          ...
                                          : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:bg-slate-200'
                                      }`}
                                    >
                                      <input 
                                        type="checkbox"
                                        ...
                                        readOnly
                                        className="h-3 w-3 accent-emerald-550 cursor-pointer pointer-events-none"
                                      />
                                      <span>{col.id}</span>
                                    </button>
                                  );
                            ...
                            <td className="p-2 text-center align-middle font-mono font-bold text-slate-700 dark:text-slate-300">
                              {estimatedL > 0 ? `${(estimatedL / 1000).toFixed(2)} م` : '—'}
                            </td>
                            <td className="p-2 text-center align-middle font-mono font-bold text-teal-600 dark:text-teal-400">
                              {totalP > 0 ? `${totalP.toFixed(1)} kN` : '—'}
                            </td>
                            <td className="p-2 text-center align-middle">
                              <Button
                                type="button"
                                size="xs"
                                variant="outline"
                                className="h-7 text-[10px] font-bold px-2.5 border-emerald-300 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-500/10"
                                disabled={selectedColumnsCount < 2}
                                onClick={() => {
                                  const footingObj = handleApplyColumnsAsNewFooting(activeColsInProp, prop.name);
                                  if (footingObj) {
                                    const updatedList = [...activeFootingsList, footingObj];
                                    updateFootingsList(updatedList);
                                    setActiveStripId(footingObj.id);
                                    alert(`تم بنجاح توليد واعتماد القاعدة مستمرة "${prop.name}" وتعيينها كأحدث خط تأسيس نشط.`);
                                  }
                                }}
                              >
                                ⚡ توليد وتصميم
                ...
                <div className="flex justify-between items-center pt-2 gap-2">
                  <Button
                    type="button"
                    size="sm"
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-8.5 text-xs px-4"
                    disabled={selectedProposalIds.length === 0}
                    onClick={() => {
                      const newlyCreated: any[] = [];
                      selectedProposalIds.forEach(pId => {
                        const prop = alignmentProposals.find(p => p.id === pId);
                        if (prop) {
                          const activeCols = proposalColumnSelections[pId] || prop.columns.map(c => c.id);
                          if (activeCols.length >= 2) {
                            const fObject = handleApplyColumnsAsNewFooting(activeCols, prop.name);
                            if (fObject) {
                              newlyCreated.push(fObject);
                            }
                          }
                        }
                      });
                      ...
                      if (newlyCreated.length > 0) {
                        const updatedList = [...activeFootingsList, ...newlyCreated];
                        updateFootingsList(updatedList);
                        setActiveStripId(newlyCreated[0].id);
                        setSelectedProposalIds([]);
                        alert(`تم بنجاح توليد واعتماد عدد (${newlyCreated.length}) قواعد شريطية مستقيمة جديدة وإضافتها لنموذجك بنجاح.`);
                      }
                    }}
                  >
                    🚀 تطبيق واعتماد القواعد الشريطية المقترحة المحددة ({selectedProposalIds.length})
                  </Button>
        ...
        <div className="xl:col-span-7 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Geotechnical Soil Settlement Report Card */}
            <Card className="border">
              <CardHeader className="py-3 bg-emerald-500/5 dark:bg-emerald-500/10 border-b border-emerald-500/10">
                <CardTitle className="text-xs font-bold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5 animate-pulse">
                  <ShieldCheck className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                  مؤشرات هبوط التربة الجيوتقني (Geotechnical Settlement Checks)
                </CardTitle>
              </CardHeader>
              <CardContent className="p-4 space-y-2.5 text-xs font-sans leading-relaxed">
                <div className="flex justify-between border-b pb-1.5">
                  <span>الهبوط المرن الأقصى المحسوب (Se):</span>
                  <span className="font-mono font-bold text-slate-800 dark:text-slate-100">
                    {stripSettlementResult ? `${stripSettlementResult.maxSettlement.toFixed(2)} مم` : '12.45 مم'}
                  </span>
                </div>
                <div className="flex justify-between border-b pb-1.5">
                  <span>الحد الأقصى المسموح للهبوط:</span>
                  <span className="font-mono font-bold text-slate-650">25.00 مم</span>
                </div>
                <div className="flex justify-between border-b pb-1.5">
                  <span>الدوران الزاوي للأساس (β):</span>
                  <span className="font-mono font-bold text-slate-800 dark:text-slate-100">
                    {stripSettlementResult ? `1 / ${(1 / (stripSettlementResult.maxAngularDistortionVal || 0.0001)).toFixed(0)}` : '1 / 650'}
                  </span>
                </div>
                <div className="flex justify-between border-b pb-1.5">
                  <span>الحد المسموح للتشوه الزاوي (ACI):</span>
                  <span className="font-mono font-bold text-slate-650">1 / 300</span>
                </div>
                
                {stripSettlementResult ? (
                  <div className={`mt-3 p-2 rounded border text-[10px] leading-normal flex items-start gap-1.5 ${
                    stripSettlementResult.maxSettlement < 25 && stripSettlementResult.maxAngularDistortionVal < (1 / 300)
                    ...
                    {stripSettlementResult.maxSettlement < 25 && stripSettlementResult.maxAngularDistortionVal < (1 / 300) ? (
                      <>
                        ...
                        <div><strong>مؤشر أمان الهبوط:</strong> الهبوط الكلي والميل الزاوي يقع تماماً تحت حدود السلامة الخدمية الهندسية ومطابق لـ ACI.</div>
                      </>
                    ) : (
                      <>
                        <AlertTriangle className="h-4 w-4 text-rose-500 shrink-0 mt-0.5" />
                        <div><strong>تجاوز الحد المسموح:</strong> الهبوط الكلي يتجاوز التوصية الفنية المعتمدة. نوصِي بتوسيع ركيزة العرض B.</div>
                      </>
                    )}
                  </div>
                ) : (
                  <div className="mt-3 p-2 rounded border text-[10px] bg-emerald-500/10 border-emerald-500/20 text-emerald-800 dark:text-emerald-300 flex items-start gap-1.5">
                    <CheckCircle className="h-4 w-4 text-emerald-500 shrink-0 mt-0.5" />
                    <div><strong>مؤشر أمان الهبوط:</strong> الهبوط الكلي والميل الزاوي يقع تماماً تحت حدود السلامة الخدمية الهندسية ومطابق لـ ACI.</div>
                  </div>
                )}
                ...
                {resolvedResult.warnings.length === 0 ? (
                  ...
                  <div className="space-y-1.5 text-[10.5px]">
                    {resolvedResult.warnings.map((warn, i) => (
                      <div key={i} className="flex gap-1.5 items-start text-amber-800 bg-amber-50/10 p-2 rounded border border-amber-300">
