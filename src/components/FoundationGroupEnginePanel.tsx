...
interface FoundationGroupEnginePanelProps {
  ...
  onConfigApplied?: (types: FoundationType[], instances: any[]) => void;
}
            ...
            <CardContent className="space-y-4 text-xs">
              ...
              {onConfigApplied && (
                <div className="pt-3 border-t border-border mt-3 space-y-2">
                  <Button 
                    type="button"
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs gap-1.5 h-10 shadow cursor-pointer"
                    onClick={() => {
                      onConfigApplied(types, classifiedInstances); 
                      triggerBanner('success', '✅ تم تطبيق معايير وتوحيد القواعد ومزامنة أبعاد الأوعية بنجاح!');
                    }}
                  >
                    <ListChecks className="h-4 w-4" />
                    تطبيق وتوحيد أبعاد كافّة القواعد الفعلية 💾
                  </Button>
                  <p className="text-[10px] text-muted-foreground text-center">
                    سيقوم هذا بتعديل القواعد النشطة لتتوافق مع أبعاد النماذج المعيارية الموحدة.
                  </p>
                </div>
              )}
