...
interface ModelCanvasProps {
  ...
  onEmptyLongPress?: (x: number, y: number) => void;
...
export default function ModelCanvas({
  ...
  onFrameLongPress, onAreaLongPress, onNodeLongPress, onEmptyLongPress,
  ...
  const bgTouchStartPos = useRef<{ x: number; y: number } | null>(null);
  const elemTouchStartPos = useRef<{ x: number; y: number } | null>(null);
  ...
  useEffect(() => {
    ...
    // Flip Y: SVG viewBox Y goes from -maxY to -minY (negated world coords)
    setViewBox({ x: minX, y: -maxY, w: maxX - minX, h: maxY - minY });
  }, [nodes]);
  ...
  const screenToWorld = useCallback((clientX: number, clientY: number) => {
    ...
    // Flip Y: screen top = max Y, screen bottom = min Y
    const svgY = (clientY - rect.top) * scaleY + viewBox.y;
    const y = -(svgY); // negate to get world Y (bottom-up)
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent<SVGSVGElement>) => {
    // Cancel bg long-press immediately on multi-touch (pinch)
    if (e.touches.length > 1) {
      if (bgLongPressTimer.current) { clearTimeout(bgLongPressTimer.current); bgLongPressTimer.current = null; }
      bgTouchStartPos.current = null;
      return;
    }
    // Single touch reaching SVG = empty canvas space (element touches stop propagation)
    const touch = e.touches[0];
    bgTouchStartPos.current = { x: touch.clientX, y: touch.clientY };
    if (bgLongPressTimer.current) clearTimeout(bgLongPressTimer.current);
    bgLongPressTimer.current = setTimeout(() => {
      if (bgTouchStartPos.current) {
        const pos = screenToWorld(bgTouchStartPos.current.x, bgTouchStartPos.current.y);
        onEmptyLongPress?.(pos.x, pos.y);
      }
      bgTouchStartPos.current = null;
    }, 700);
    // Also start panning
  ...
  }, [activeTool, viewBox, screenToWorld, onEmptyLongPress]);

  const handleTouchMove = useCallback((e: React.TouchEvent<SVGSVGElement>) => {
    // Cancel bg long-press on any significant movement or multi-touch
    if (e.touches.length >= 2) {
      if (bgLongPressTimer.current) { clearTimeout(bgLongPressTimer.current); bgLongPressTimer.current = null; }
      bgTouchStartPos.current = null;
    } else if (e.touches.length === 1 && bgTouchStartPos.current) {
      const touch = e.touches[0];
      const dx = Math.abs(touch.clientX - bgTouchStartPos.current.x);
      const dy = Math.abs(touch.clientY - bgTouchStartPos.current.y);
      if (dx > 12 || dy > 12) {
        if (bgLongPressTimer.current) { clearTimeout(bgLongPressTimer.current); bgLongPressTimer.current = null; }
        bgTouchStartPos.current = null;
      }
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent<SVGSVGElement>) => {
    // Cancel bg long-press on any finger lift
    if (bgLongPressTimer.current) { clearTimeout(bgLongPressTimer.current); bgLongPressTimer.current = null; }
    bgTouchStartPos.current = null;
  ...
  return (
    <div className="canvas-container flex-1 relative touch-none">
      ...
      >
        {/* Transparent background — desktop mouse long-press for empty canvas */}
        <rect
          ...
          onMouseUp={() => { if (bgLongPressTimer.current) { clearTimeout(bgLongPressTimer.current); bgLongPressTimer.current = null; } }}
          onMouseLeave={() => { if (bgLongPressTimer.current) { clearTimeout(bgLongPressTimer.current); bgLongPressTimer.current = null; } }}
        />
        ...
        {/* Area elements (slabs) - Y negated for bottom-left origin */}
        {visibleTypes.slabs && areas.map(area => {
          ...
          return (
            <g key={`a${area.id}`}
              ...
              onMouseDown={(e) => { e.stopPropagation(); longPressTimer.current = setTimeout(() => { onAreaLongPress?.(area.id); }, 500); }}
              onMouseUp={(e) => { e.stopPropagation(); if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onMouseLeave={() => { if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onTouchStart={(e) => { e.stopPropagation(); const t=e.touches[0]; elemTouchStartPos.current={x:t.clientX,y:t.clientY}; longPressTimer.current = setTimeout(() => { onAreaLongPress?.(area.id); }, 500); }}
              onTouchEnd={(e) => { e.stopPropagation(); elemTouchStartPos.current=null; if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onTouchMove={(e) => { if (elemTouchStartPos.current && longPressTimer.current) { const t=e.touches[0]; if(Math.abs(t.clientX-elemTouchStartPos.current.x)>10||Math.abs(t.clientY-elemTouchStartPos.current.y)>10){clearTimeout(longPressTimer.current);longPressTimer.current=null;elemTouchStartPos.current=null;} } }}
            >
        ...
        {/* Beam elements */}
        {visibleTypes.beams && beamFrames.map(frame => {
          const ni = getNodeById(frame.nodeI);
          const nj = getNodeById(frame.nodeJ);
          if (!ni || !nj) return null;
          const isSelected = selectedFrameId === frame.id;
          // Check for end releases
          let hasReleaseI = false;
          let hasReleaseJ = false;
          if (frameEndReleases) {
            const posKey = `${ni.x.toFixed(3)}_${ni.y.toFixed(3)}_${nj.x.toFixed(3)}_${nj.y.toFixed(3)}`;
            const posKeyRev = `${nj.x.toFixed(3)}_${ni.y.toFixed(3)}_${ni.x.toFixed(3)}_${nj.y.toFixed(3)}`;
            const rel = frameEndReleases[posKey] || frameEndReleases[`${nj.x.toFixed(3)}_${nj.y.toFixed(3)}_${ni.x.toFixed(3)}_${ni.y.toFixed(3)}`];
            if (rel) {
              const isRev = !frameEndReleases[posKey];
              const rI = isRev ? rel.nodeJ : rel.nodeI;
              const rJ = isRev ? rel.nodeI : rel.nodeJ;
              hasReleaseI = rI.rx || rI.ry || rI.rz;
              hasReleaseJ = rJ.rx || rJ.ry || rJ.rz;
            }
          }
          return (
            <g key={`f${frame.id}`}
              onClick={(e) => { e.stopPropagation(); onFrameClick(frame.id); }}
              onMouseDown={(e) => { e.stopPropagation(); longPressTimer.current = setTimeout(() => { onFrameLongPress?.(frame.id); }, 500); }}
              onMouseUp={(e) => { e.stopPropagation(); if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onMouseLeave={() => { if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onTouchStart={(e) => { e.stopPropagation(); const t=e.touches[0]; elemTouchStartPos.current={x:t.clientX,y:t.clientY}; longPressTimer.current = setTimeout(() => { onFrameLongPress?.(frame.id); }, 500); }}
              onTouchEnd={(e) => { e.stopPropagation(); elemTouchStartPos.current=null; if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onTouchMove={(e) => { if (elemTouchStartPos.current && longPressTimer.current) { const t=e.touches[0]; if(Math.abs(t.clientX-elemTouchStartPos.current.x)>10||Math.abs(t.clientY-elemTouchStartPos.current.y)>10){clearTimeout(longPressTimer.current);longPressTimer.current=null;elemTouchStartPos.current=null;} } }}
            >
              ...
              {/* Pin indicators for released ends */}
              {hasReleaseI && (
                <circle cx={ni.x} cy={-ni.y} r="0.12"
                  fill="hsl(var(--background))" stroke="hsl(var(--accent))" strokeWidth="0.03" />
              )}
              {hasReleaseJ && (
                <circle cx={nj.x} cy={-nj.y} r="0.12"
                  fill="hsl(var(--background))" stroke="hsl(var(--accent))" strokeWidth="0.03" />
              )}
        ...
        {visibleTypes.nodes && floorNodes.map(node => {
          ...
          return (
            <g key={`n${node.id}`}
              ...
              onMouseDown={(e) => { e.stopPropagation(); longPressTimer.current = setTimeout(() => { onNodeLongPress?.(node.id); }, 500); }}
              onMouseUp={(e) => { e.stopPropagation(); if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onMouseLeave={() => { if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onTouchStart={(e) => { e.stopPropagation(); const t=e.touches[0]; elemTouchStartPos.current={x:t.clientX,y:t.clientY}; longPressTimer.current = setTimeout(() => { onNodeLongPress?.(node.id); }, 500); }}
              onTouchEnd={(e) => { e.stopPropagation(); elemTouchStartPos.current=null; if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } }}
              onTouchMove={(e) => { if (elemTouchStartPos.current && longPressTimer.current) { const t=e.touches[0]; if(Math.abs(t.clientX-elemTouchStartPos.current.x)>10||Math.abs(t.clientY-elemTouchStartPos.current.y)>10){clearTimeout(longPressTimer.current);longPressTimer.current=null;elemTouchStartPos.current=null;} } }}
            >
              <circle cx={node.x} cy={ny} r={isSelected ? 0.15 : 0.1}
                className="element-node"
                style={isSelected ? { stroke: 'hsl(var(--accent))', strokeWidth: '0.06', fill: 'hsl(var(--accent))' } : {}} />
              {!hasColumn && (
                <text x={node.x} y={ny + 0.2} textAnchor="middle"
                  className="fill-foreground" fontSize="0.2" fontFamily="JetBrains Mono">
                  N{node.id}
                </text>
              )}
              {hasRestraint && (
                <polygon points={`${node.x},${ny - 0.1} ${node.x - 0.12},${ny - 0.25} ${node.x + 0.12},${ny - 0.25}`}
        ...
        {visibleTypes.columns && columnFrames.map(frame => {
          ...
          return (
            <g key={`c${frame.id}`}
              ...
              onTouchMove={(e) => { if (elemTouchStartPos.current && longPressTimer.current) { const t=e.touches[0]; if(Math.abs(t.clientX-elemTouchStartPos.current.x)>10||Math.abs(t.clientY-elemTouchStartPos.current.y)>10){clearTimeout(longPressTimer.current);longPressTimer.current=null;elemTouchStartPos.current=null;} } }}
            >
              <rect x={topNode.x - 0.25} y={ny - 0.25} width="0.5" height="0.5"
                fill="transparent" />
              <rect x={topNode.x - 0.15} y={ny - 0.15} width="0.3" height="0.3"
                className="element-column" rx="0.02"
                style={isSelected ? { stroke: 'hsl(var(--accent))', strokeWidth: '0.06' } : {}} />
              <text x={topNode.x} y={ny - 0.35} textAnchor="middle"
