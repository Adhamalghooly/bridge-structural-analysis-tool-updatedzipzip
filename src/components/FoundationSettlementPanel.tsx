...
import type { FootingDesignResult } from '@/lib/foundationDesign';
...
interface Props {
  ...
  defaultQall?: number;
  /** USD design results — used to auto-populate footing dimensions in settlement analysis */
  batchResults?: FootingDesignResult[];
}
...
export default function FoundationSettlementPanel({
  ...
  const groundCols = useMemo(() => {
    return columns.filter(c => {
      ...
      return Math.abs((c.zBottom ?? 0) - minZ) < 100;
    });
  }, [columns]);

  const handleImportColumnLoads = () => {
    if (groundCols.length === 0) return;
    // Pick the governing (heaviest) column for the isolated settlement check
    // The audit noted this only imports one column — that is intentional for the
    // isolated footing tab (it shows one representative analysis). All columns
    // can be checked manually by the user by changing isoName via the dropdown.
    ...
    if (loads) {
      ...
      setIsoName(`قاعدة العمود ${heaviestCol.id} (أعلى حمل)`);
    }

    // Also import the footing dimensions from USD design results if available
    if (batchResults.length > 0) {
      const matchingFooting = batchResults.find(r => r.colId === heaviestCol.id)
        ?? batchResults.reduce((prev, curr) =>
          curr.P_service > prev.P_service ? curr : prev, batchResults[0]);
      if (matchingFooting) {
        setIsoB(parseFloat((matchingFooting.B / 1000).toFixed(2)));  // mm → m
        setIsoL(parseFloat((matchingFooting.L / 1000).toFixed(2)));
        setIsoH(parseFloat((matchingFooting.t / 1000).toFixed(2)));
      }
    }
  };

  /** Import footing plan dimensions from USD design results (without changing the load) */
  const handleImportFootingDimensions = () => {
    if (batchResults.length === 0) return;
    // Find the footing matching the currently named column, or pick the heaviest
    const named = batchResults.find(r =>
      isoName.includes(r.colId) || r.colId.toLowerCase() === isoName.toLowerCase()
    );
    const footing = named ?? batchResults.reduce((prev, curr) =>
      curr.P_service > prev.P_service ? curr : prev, batchResults[0]);
    setIsoB(parseFloat((footing.B / 1000).toFixed(2)));
    setIsoL(parseFloat((footing.L / 1000).toFixed(2)));
    setIsoH(parseFloat((footing.t / 1000).toFixed(2)));
    setIsoP(parseFloat(footing.P_service.toFixed(1)));
    setIsoName(`قاعدة ${footing.colId} (من تصميم USD)`);
  };

  const handleExportSettlement = () => {
    const result = activeSubTab === 'isolated' ? isolatedResult
      : activeSubTab === 'strip' ? stripResult
      : activeSubTab === 'combined' ? combinedResult
      : raftResult;
    const rows = result.points.map((p: any) =>
      `${p.label ?? p.id ?? ''},${Number(p.settlement ?? 0).toFixed(2)},${Number(p.x ?? 0).toFixed(3)},${Number(p.y ?? 0).toFixed(3)}`
    );
    const csv = [
      'نوع الأساس,نقطة القياس,الهبوط (mm),X (m),Y (m),الهبوط الأقصى (mm),الهبوط التفاضلي (mm),حالة الخدمة',
      ...rows.map(row => `${activeSubTab},${row},${result.maxSettlement.toFixed(2)},${result.differentialSettlement.toFixed(2)},${result.isSafe ? 'آمن' : 'تجاوز'}`)
    ].join('\n');
    downloadCSV(`foundation_settlement_${activeSubTab}.csv`, csv);
  };
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                className="text-xs flex items-center gap-1.5 border-emerald-400 text-emerald-700 hover:bg-emerald-50"
                onClick={handleImportFootingDimensions}
                disabled={batchResults.length === 0}
                title={batchResults.length === 0 ? 'لا توجد نتائج تصميم قواعد متاحة — شغّل تصميم القواعد USD أولاً' : 'استيراد أبعاد القاعدة B×L×t من نتائج تصميم USD'}
              >
                <Database className="h-3.5 w-3.5" />
                استيراد أبعاد من تصميم USD ({batchResults.length} قاعدة)
              </Button>
              <Button
                variant="outline"
                size="sm"
                className="text-xs flex items-center gap-1.5 border-sky-400 text-sky-700 hover:bg-sky-50"
                onClick={handleExportSettlement}
              >
                <Database className="h-3.5 w-3.5" />
                تصدير نتائج الهبوط CSV
              </Button>
            </div>
          </div>
        </CardContent>
