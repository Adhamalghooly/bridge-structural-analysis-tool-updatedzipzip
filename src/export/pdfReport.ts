...
export function generateStructuralReport(
  ...
  const getBeamDisplayName = (d: { beamId: string; mergedCarrierIds?: string[] } | string): string => {
    const beamId = typeof d === 'string' ? d : d.beamId;
    const mergedIds = typeof d === 'string' ? undefined : d.mergedCarrierIds;
    
    if (mergedIds && mergedIds.length >= 2) {
      const parts = mergedIds.map(id => beams.find(b => b.id === id)).filter(Boolean);
    ...
    const parts = beams.filter(x => x.id.startsWith(beamId + '-'));
    if (parts.length > 0) {
      const namedPart = parts.find(p => p.name);
      if (namedPart && namedPart.name) {
        return namedPart.name.replace(/-\d+$/, '');
  ...
  for (const fr of frameResults) {
    ...
    autoTable(doc, {
      ...
      body: fr.beams.map(b => [
        getBeamDisplayName(b.beamId), b.span.toFixed(1),
  ...
  autoTable(doc, {
    ...
    body: beamDesigns.map(d => {
      ...
      return [
        getBeamDisplayName(d), beta1.toFixed(3), rhoMin.toFixed(5), rhoMax.toFixed(5),
  ...
  autoTable(doc, {
    ...
    body: beamDesigns.map(d => [
      getBeamDisplayName(d), d.Vu.toFixed(1),
  ...
  autoTable(doc, {
    ...
    body: beamDesigns.map(d => [
      getBeamDisplayName(d), d.deflection.deflection.toFixed(1),
  ...
  autoTable(doc, {
    ...
    body: beamDesigns.map(d => {
      const beam = beams.find(b => b.id === d.beamId) || (d.mergedCarrierIds && d.mergedCarrierIds.map(id => beams.find(b => b.id === id)).find(Boolean));
      return [
        getBeamDisplayName(d), `${beam?.b}×${beam?.h}`,
  ...
  // ========== 11. FOUNDATION DESIGN SUMMARY ==========
  if (foundationResults && foundationResults.length > 0) {
    doc.addPage(); y = margin;
    addTitle('11. ISOLATED FOOTING DESIGN SUMMARY (ACI 318-19)', 16); addLine();
    addText(`f'c = ${mat.fc} MPa   |   fy = ${mat.fy} MPa   |   USD Method (φ-factors applied)`);
    y += 4;

    autoTable(doc, {
      startY: y,
      head: [['Column', 'P_svc (kN)', 'B (mm)', 'L (mm)', 't (mm)', 'd (mm)',
              'Bearing OK', 'Wide Shear OK', 'Punch OK',
              'Bot Bars X', 'Bot Bars Y']],
      body: foundationResults.map((r: any) => [
        r.colId ?? '-',
        (r.P_service ?? 0).toFixed(0),
        (r.B ?? 0).toFixed(0),
        (r.L ?? 0).toFixed(0),
        (r.t ?? 0).toFixed(0),
        (r.d ?? 0).toFixed(0),
        r.bearing_ok ? '✓' : '✗',
        r.wide_shear_ok ? '✓' : '✗',
        r.punch_ok ?? r.punching_ok ?? (r.Vc_punch > r.Vu_punch ? '✓' : '✗'),
        `${r.bars_x ?? '-'}Φ${r.dia_x ?? '-'}@${r.spacing_x ?? '-'}mm`,
        `${r.bars_y ?? '-'}Φ${r.dia_y ?? '-'}@${r.spacing_y ?? '-'}mm`,
      ]),
      margin: { left: margin },
      styles: { fontSize: 7 },
      headStyles: { fillColor: [64, 82, 60] },
      alternateRowStyles: { fillColor: [240, 245, 240] },
    });

    y = (doc as any).lastAutoTable.finalY + 6;
    addText('Note: Dimensions in mm. Service load shown; USD design uses Pu = 1.2D + 1.6L with φ = 0.75 (shear), φ = 0.90 (flexure).');
  }
