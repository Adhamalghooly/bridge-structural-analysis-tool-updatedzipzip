...
export function solveFootingSizing(
  ...
  const solveOption = (
    ...
    const isFeasible = finalAnalysis.soilPressure.qmax <= qall
      && finalAnalysis.criticalSections.punching_ok
      && finalAnalysis.criticalSections.wideShearX_ok
      && finalAnalysis.criticalSections.wideShearY_ok;
    ...
    return {
      ...
      isFeasible,
      warningAr: isFeasible ? undefined : `الأبعاد ${B}×${L}×${H} مم تعطي qu=${finalAnalysis.soilPressure.qmax.toFixed(0)} > qall=${qall} أو فشل في قص`,
    };
  };
  ...
  const solveExcelDirect = (): SizingResultOption => {
    ...
    const isFeasible_ed = finalAnalysis_ed.soilPressure.qmax <= qall
      && finalAnalysis_ed.criticalSections.punching_ok
      && finalAnalysis_ed.criticalSections.wideShearX_ok
      && finalAnalysis_ed.criticalSections.wideShearY_ok;
    ...
    return {
      ...
      isFeasible: isFeasible_ed,
      warningAr: isFeasible_ed ? undefined : `الأبعاد ${B_ed}×${L_ed}×${H_ed} مم تعطي qu=${finalAnalysis_ed.soilPressure.qmax.toFixed(0)} > qall=${qall} أو فشل في قص`,
    };
  };
