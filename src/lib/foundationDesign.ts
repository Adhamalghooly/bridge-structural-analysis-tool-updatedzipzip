/**
 * Foundation Design — ACI 318-19 Ultimate Strength Design (USD)
 *
 * NOTE: This module used to implement the obsolete Working Stress Method
 * (WSM/ASD, removed from ACI since 2002) independently from the rest of the
 * app (beams/columns/slabs all use USD). `designFooting` below is now a thin
 * adapter around the validated USD engine in `isolatedFootingEngine.ts` /
 * `isolatedFootingDesignEngine.ts` (the same engine used by the interactive
 * per-column design view), so every consumer of `FootingDesignResult`
 * (BOQ, reinforcement schedule, DXF/PDF export, drawings) now reflects one
 * single, correct source of truth instead of a separate/older calculation.
 * Footing plan sizing (B×L vs. allowable soil bearing) still correctly uses
 * service-level loads; everything downstream (flexure/shear/punching design)
 * now uses factored (Pu) strength design with φ factors.
 */
import { analyzeIsolatedFooting, IsolatedFootingAnalysisResult } from './isolatedFootingEngine';
import { designIsolatedFootingStrength, IsolatedFootingDesignOutput } from './isolatedFootingDesignEngine';
...
export interface ColumnReactionInput {
  ...
  manualB?: number; // manual width override mm (x-direction)
  manualL?: number; // manual length override mm (y-direction)
  manualH?: number; // manual thickness override mm
}
...
export interface FootingDesignResult {
  ...
  // Legacy WSM constants — retained for backward type-compatibility only, no
  // longer used to derive any design quantity (all USD/φ-based now).
  fc_allow: number;
  fs_allow: number;
  n: number;
  k: number;
...
/**
 * Design isolated spread footing per ACI 318-19 Ultimate Strength Design.
 *
 ...
 *
 * Sizing (B×L) is governed by service-level soil bearing (correct — soil
 * capacity is a service-load / allowable-stress check, not a strength check).
 * Thickness, flexural reinforcement, one-way shear, and punching shear are all
 * governed by factored (Pu) strength design via `analyzeIsolatedFooting` +
 * `designIsolatedFootingStrength` — the same validated engine used by the
 * interactive per-column design view.
 */
export function designFooting(
  ...
  const Mx_service = (reaction.Mx_DL ?? 0) + (reaction.Mx_LL ?? 0);
  const My_service = (reaction.My_DL ?? 0) + (reaction.My_LL ?? 0);

  const P_service = P_DL + P_LL;
  // Effective ultimate/service ratio derived from the ACTUAL D/L split supplied
  // by the caller (1.2·D + 1.6·L per ACI 318-19 §5.3.1), instead of a flat
  // assumed factor or an arbitrary 60/40 D/L guess.
  const effLoadFactor = P_service > 0.01 ? (1.2 * P_DL + 1.6 * P_LL) / P_service : 1.5;
  ...
  const autoSize = reaction.manualB === undefined || reaction.manualL === undefined;
  const autoThickness = reaction.manualH === undefined;

  let t = reaction.manualH ?? t_min_aci;
  let B = reaction.manualB ?? Math.max(roundUpTo(Math.sqrt((P_service / qa) / aspect) * 1000, 50), colB + 400);
  let L = reaction.manualL ?? Math.max(roundUpTo(Math.sqrt((P_service / qa) * aspect) * 1000, 50), colH + 400);

  let analysis: IsolatedFootingAnalysisResult;
  let design: IsolatedFootingDesignOutput;
  ...
  for (let iter = 0; iter < 20; iter++) {
    ...
    analysis = analyzeIsolatedFooting({
      B, L, H: t, Cx: colB, Cy: colH, fxCol: 0, fyCol: 0,
      fc, qall: qa,
      includeSelfWeight: true, includeSoilCover: true, soilCoverDepth: Df,
      gammaConc: gamma_conc, gammaSoil: gamma_soil,
      P: P_service, Mx: Mx_service, My: My_service, Vx: 0, Vy: 0,
    });

    design = designIsolatedFootingStrength(analysis, fy, effLoadFactor, cover);

    const bearingOk = analysis.bearingStatus === 'pass';
    const shearOk = design.shearX.isSafe && design.shearY.isSafe;
    const punchOk = design.punching.isSafe;

    if ((bearingOk || !autoSize) && shearOk && punchOk) break;
    if (!autoThickness) break; // manual thickness override — report inadequacy, don't resize

    t = Math.max(t_min_aci, roundUpTo(t + 50, 50));
  }

  // ── Map the strength-design output back onto the legacy result shape ──────
  // (kept stable so BOQ / reinforcement schedule / DXF & PDF export / drawing
  // generation all continue to work unchanged, now fed correct USD numbers).
  const a_x = (B - colB) / 2;
  const a_y = (L - colH) / 2;
  const d = Math.max(100, t - cover - 12);

  const q_actual = analysis!.soilPressure.qavg;
  const q_net_allow = qa - (gamma_soil * Math.max(0, Df - t / 1000) + gamma_conc * (t / 1000));
  const bearing_ok = analysis!.bearingStatus === 'pass';

  const Vu_wide = Math.max(design!.shearX.Vu, design!.shearY.Vu);
  const Vc_wide = Math.min(design!.shearX.phiVc, design!.shearY.phiVc);
  const wide_shear_ok = design!.shearX.isSafe && design!.shearY.isSafe;

  const Vu_punch = design!.punching.Vu;
  const Vc_punch = (design!.punching.phiVcStress * design!.punching.Area) / 1000; // MPa·mm² → kN
  const punch_shear_ok = design!.punching.isSafe;
  ...
  return {
    ...
    q_actual,
    bearing_ok,
    M_x: design!.flexureX.MuPerMeter,
    As_x_req: design!.flexureX.AsRequiredPerMeter,
    As_x_use: design!.flexureX.AsProvided / (L / 1000),
    bars_x: design!.flexureX.selectedQuantity, dia_x: design!.flexureX.selectedDiameter, spacing_x: design!.flexureX.selectedSpacing,
    M_y: design!.flexureY.MuPerMeter,
    As_y_req: design!.flexureY.AsRequiredPerMeter,
    As_y_use: design!.flexureY.AsProvided / (B / 1000),
    bars_y: design!.flexureY.selectedQuantity, dia_y: design!.flexureY.selectedDiameter, spacing_y: design!.flexureY.selectedSpacing,
    Vu_wide, Vc_wide, wide_shear_ok,
    Vu_punch, Vc_punch, punch_shear_ok,
    // Legacy WSM constants — retained only for backward type-compatibility; no
    // longer used to derive any design quantity above (all USD/φ-based now).
    fc_allow: 0.45 * fc, fs_allow: Math.min(0.50 * fy, 207), n: 6, k: 0.4, j: 0.87,
    As_min_pm: design!.flexureX.AsMinPerMeter,
    a_x, a_y,
    colB, colH,
    t_min_aci,
    adequate: bearing_ok && wide_shear_ok && punch_shear_ok && design!.summary.overallStatus === 'pass',
  };
}
